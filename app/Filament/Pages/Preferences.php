                                ...
                                Section::make(__('Casting Transcoding'))
                                    ...
                                    ->schema([
                                        Select::make('default_cast_stream_profile_id')
                                            ->label(__('Cast Live Transcoding Profile'))
                                            ->searchable()
                                            ->options(function () {
                                                return StreamProfile::where('user_id', auth()->id())->pluck('name', 'id');
                                            })
                                            ...
                                            ->helperText(new HtmlString(__('The transcoding profile used when casting live streams. <strong>Must be an HLS (m3u8) profile</strong> — non-HLS profiles will be ignored. Leave empty to fall back to the in-app player profile.'))),
                                        Select::make('default_cast_vod_stream_profile_id')
                                            ->label(__('Cast VOD and Series Transcoding Profile'))
                                            ->searchable()
                                            ->options(function () {
                                                return StreamProfile::where('user_id', auth()->id())->pluck('name', 'id');
                                            })
                                            ...
                                            ->helperText(new HtmlString(__('The transcoding profile used when casting VOD/Series streams. <strong>Must be an HLS (m3u8) profile</strong> — non-HLS profiles will be ignored. Leave empty to fall back to the in-app player profile.'))),
                                    ]),
